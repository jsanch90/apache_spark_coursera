...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    val questions = postings.filter (post => post.postingType == 1).map(ques => (ques.id, ques))
    val answers = postings.filter(post => post.postingType == 2).map(ans => (ans.parentId.get, ans))
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped.values.map (pairs =>
      {
        val unzipp = pairs unzip
        val answers = unzipp._2.toArray
        val questions = unzipp._1.toArray
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langPercent: Double = vs.filter(_._1 == langMode).size / clusterSize * 100
