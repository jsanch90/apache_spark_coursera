...
object WikipediaRanking {
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.map(item => (item._1,item._2.size)).sortBy(_._2).collect.toList.reverse
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(art => langs.filter(lan => art.mentionsLanguage(lan)).map(lang => (lang,1)))
  ...
  def main(args: Array[String]) {
    ...
    //wikiRdd.foreach(x => println(x.text))
    //println(wikiRdd.filter(x => x.mentionsLanguage("Python")).count())
